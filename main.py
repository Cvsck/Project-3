...
def main():
    ...
    # Подключение к БД
    db = DBManager()
    db.create_tables()  # ✅ Создание таблиц перед обработкой данных
    ...
    for company_id in company_ids:
        ...
        if vacancies:
            ...
            # ✅ Вставляем вакансии в базу только если есть `company_id`
            for vacancy in parsed_vacancies:
                salary_value = vacancy["salary"] if vacancy["salary"] is not None else 0  # ✅ Заменяем NULL на 0
                if company_db_id:  # ✅ Исключаем вакансии без `company_id`
                    db.cursor.execute("""
    ...
    db.conn.commit()  # ✅ Сохранение изменений в БД
